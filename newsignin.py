...
def load_user_data():
    global user_data  # Declare user_data as a global variable
    ...
    try:
        ...
        cursor.execute(query, (username,))  # Assuming 'username' is a global variable holding the logged-in user's username
        user_data = cursor.fetchone()  # Fetch the user data
    except (Exception, pg.DatabaseError) as error:
        messagebox.showerror('Error', f'Failed to fetch user data: {error}')
    finally:
    ...
    if usernameEntry.get() == 'admin' and passwordEntry.get() == 'admin':
        ...
        if usernameEntry.get()=='' or passwordEntry.get()=='':
            ...
            if row == None:
                ...
                username = row[0]  # Store the logged-in user's username in the global variable
                load_user_data()  # Call load_user_data with the logged-in user's username
                user_page() # Navigate to the user page

def user_page():
    login_window.destroy()
    import Userpage  # Redirect to another page (Userpage.py)

def admin_page():
    login_window.destroy()
    import admin  # Redirect to another page (admin.py)

def signup_page():
    login_window.destroy()
    import signup  # Redirect to another page (signup.py)
...
login_window.after(1, load_user_data)
